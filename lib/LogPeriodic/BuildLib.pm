...
sub upload {
    ...
    die "unable to find ~/logp-internal directory"
        if !-d "$dir";

    sys(qq{aws s3 sync --delete s3://logp-internal/ $dir});

    sys(qq{cp -n dist/*.deb $dir/amd64/ });
    sys(qq{cd $dir ; dpkg-scanpackages amd64/ > amd64/Packages});
    sys(qq{cd $dir ; apt-ftparchive release amd64/ > amd64/Release});
    sys(qq{gpg -a -b -u 'Log Periodic Ltd.' < $dir/amd64/Release > $dir/amd64/Release.gpg});

    sys(qq{aws s3 sync --delete $dir s3://logp-internal});
}
